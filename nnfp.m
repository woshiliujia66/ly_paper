...
netState.activity{hyperParas.numLayer} = 1./(1+exp(-inputs2out));
% netState.activity{hyperParas.numLayer} = round(1./(1+exp(-inputs2out)));
assignin('base','netState',netState);

% zqh
% for i = 1:batchSize
%    netState.activity{hyperParas.numLayer}(:,i) = netState.activity{hyperParas.numLayer}(:,i) / norm(netState.activity{hyperParas.numLayer}(:,i));
% end
% zqh
...
%     loss = 0.;
%     for i = 1:batchSize
%         loss = loss + batch_y(:,i) .* netState.activity{hyperParas.numLayer}(:,i);
%     end
